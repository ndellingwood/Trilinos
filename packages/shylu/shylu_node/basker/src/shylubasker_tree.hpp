...
std::cout << " thread_array = malloc ( " << num_threads << " )" << std::endl;
  ...
  )
  {
    ...
    MALLOC_INT_RANK2DARRAY(LDENSE_NDBLOCK_ROW_COL);
    Kokkos::deep_copy(LDENSE_NDBLOCK_ROW_COL, 0);

    INT_RANK2DARRAY UDENSE_NDBLOCK_ROW_COL;
    MALLOC_INT_RANK2DARRAY(UDENSE_NDBLOCK_ROW_COL);
    ...
    Kokkos::View<pair_t**, EXE_SPACE> Lower_2dmap_by_mtx("L2dmap", nblks, nblks);
    Kokkos::View<pair_t**, EXE_SPACE> Upper_2dmap_by_mtx("U2dmap", nblks, nblks);
